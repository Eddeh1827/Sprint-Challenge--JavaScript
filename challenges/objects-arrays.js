...
console.log(`Tyrannosaurus weighed ${tyrannosaurus.weight}`);

// What was the diet of a velociraptor?
console.log(`Velociraptors were ${velociraptor.diet}`);

// How long was a stegosaurus?
console.log(`Stegosaurus was ${stegosaurus.length} long.`);

// What time period did tyrannosaurus live in?
console.log(`Tyrannosaurus lived in the ${tyrannosaurus.period} period`);
...
const uni = graduates.map(el => el.university).filter(el => el.match(/Uni/gi));
console.log(uni);
...
*/
const animalNames = [];
const scientific = zooAnimals.forEach(animal => {
  animalNames.push(
...
const populationTotal = zooAnimals.reduce(
  (acc, animal) => (acc += animal.population),
  0
);
